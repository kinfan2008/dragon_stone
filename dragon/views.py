...
def user_available(request):
    ...
    try:
        ...
        if userProfile.educationLevel == 'PHD':
            result_list =list(Experiment.objects.filter(Q(isOverdue=True),Q(language=userProfile.firstLanguage)|Q(language=None)).exclude(userId =userProfile))
        elif userProfile.educationLevel == 'Master':
            result_list = list(Experiment.objects.filter(Q(isOverdue=True),Q(language=userProfile.firstLanguage)|Q(language=None),Q(educationDemand='Master')|Q(educationDemand='Undergraduate')|Q(educationDemand=None)).exclude(userId=userProfile))
        elif userProfile.educationLevel == 'Postgraduate':
            result_list =list(Experiment.objects.filter(Q(isOverdue=True),Q(language=userProfile.firstLanguage)|Q(language=None),Q(educationDemand='Undergraduate')|Q(educationDemand=None)).exclude(userId=userProfile))
        print(result_list)
